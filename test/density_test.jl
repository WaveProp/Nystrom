...
@testset "Density test" begin
    ...
    Γ     = Geometry.boundary(Ω) |> Geometry.Domain
...
@testset "TangentialDensity test" begin
    ...
    σ = Density(mesh) do target  # density defined with a tangential field
        x = coords(target)
        cross(Geometry.normal(target), SingleLayerKernel(pde)(xout,target)*c)
    end
    tan_σ = TangentialDensity(σ)
    @test Density(tan_σ) ≈ σ

    ncross_σ = Nystrom.ncross(σ)
    ncross_tan_σ = Nystrom.ncross(tan_σ)
